...
int main(void)
{
    ...
    // Node 0: AI0, mV -> V
    g.nodes[0].id = 0;
    g.nodes[0].type = PLC_NODE_AI_IN;
    g.nodes[0].valueType = PLC_VAL_FLOAT;
    g.nodes[0].inA = -1;
    g.nodes[0].inB = -1;
    g.nodes[0].paramInt = 0;

    // Node 1: CONST_FLOAT 2.5
    g.nodes[1].id = 1;
    g.nodes[1].type = PLC_NODE_CONST_FLOAT;
    g.nodes[1].valueType = PLC_VAL_FLOAT;
    g.nodes[1].inA = -1;
    g.nodes[1].inB = -1;
    g.nodes[1].paramFloat = 2.5f;
    ...
    plc_port_test_set_ai_mv(0, 2000); // 2.0 V
    ...
    if (plc_port_test_get_do(0)) {
        printf("TEST FAILED: DO0 must be false when AI0=2.0V\n");
        return 1;
    }

    plc_port_test_set_ai_mv(0, 3000); // 3.0 V
    ...
    if (!plc_port_test_get_do(0)) {
        printf("TEST FAILED: DO0 must be true when AI0=3.0V\n");
        return 1;
    }

    printf("TEST OK: AI0 -> COMPARE_GT -> DO0\n");
    return 0;
}