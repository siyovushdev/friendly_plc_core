...
int main(void)
{
    ...
    g.nodes[0].id = 0;
    g.nodes[0].type = PLC_NODE_HSC_IN;
    ...
    if (plc_port_test_get_do(0)) {
        printf("TEST FAILED: DO0 must be false when HSC0=50\n");
        return 1;
    }

    plc_port_test_set_hsc(0, 150);
    ...
    if (!plc_port_test_get_do(0)) {
        printf("TEST FAILED: DO0 must be true when HSC0=150\n");
        return 1;
    }

    printf("TEST OK: HSC0 -> COMPARE_GT -> DO0\n");
    return 0;
}